...
variantCollection <- function(watchdogDir){
  if(dir.exists(watchdogDir)){
    ...
    if(grepl("Snvindel_watchdog", watchdogDir)){
      ...
      infopath = paste0(watchdogDir, "/GNXS_metadata.txt")
...
if(nrow(metavariants$snv) > 0){
  if(file.exists("/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_SNV.tsv")){
    readr::write_tsv(metavariants$snv, "/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_SNV.tsv", append = TRUE, col_names = TRUE)
  }else{
    readr::write_tsv(metavariants$snv, "/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_SNV.tsv", append = FALSE, col_names = TRUE)
  }
}

if(nrow(metavariants$cnv) > 0){
  if(file.exists("/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_CNV.tsv")){
    readr::write_tsv(metavariants$cnv, "/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_CNV.tsv", append = TRUE, col_names = TRUE)
  }else{
    readr::write_tsv(metavariants$cnv, "/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_CNV.tsv", append = FALSE, col_names = TRUE)
  }
}

if(nrow(metavariants$filtered) > 0){
  if(file.exists("/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_FILTERED.tsv")){
    readr::write_tsv(metavariants$filtered, "/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_FILTERED.tsv", append = TRUE, col_names = TRUE)
  }else{
    readr::write_tsv(metavariants$filtered, "/mnt/NGS_Diagnostik/SampleCentricVariantCollection/Sample_centric_FILTERED.tsv", append = FALSE, col_names = TRUE)
  }
}
